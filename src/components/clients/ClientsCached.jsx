...
const Clients = React.memo(() => {
    // Get data from context - NO MORE API CALLS!
    const { getCachedData, updateCache, fetchData, invalidateCache } = window.useData();
    ...
    // Save client (optimistic update + background sync)
    const handleSaveClient = async (clientFormData, stayInEditMode = false) => {
        try {
            ...
            if (selectedClient) {
                updateCache('clients', (clients) => 
                    clients.map(c => c.id === selectedClient.id ? comprehensiveClient : c)
                );
                setSelectedClient(comprehensiveClient);
            } else {
                updateCache('clients', (clients) => [...clients, comprehensiveClient]);
            }
            ...
            if (token) {
                try {
                    ...
                    if (selectedClient) {
                        ...
                        const created = await window.api.createClient(apiUpdateData);
                        // Update ID from server
                        if (created?.data?.client?.id) {
                            comprehensiveClient.id = created.data.client.id;
                            updateCache('clients', (clients) =>
                                clients.map(c => c.id === Date.now().toString() ? comprehensiveClient : c)
                            );
                        }
                    }
                } catch (apiError) {
                    console.error('⚠️ Background sync failed:', apiError);
                    // Data is still in cache, user can continue working
                }
            }
    ...
    // Save lead (optimistic update + background sync)
    const handleSaveLead = async (leadFormData) => {
        try {
            ...
            if (selectedLead) {
                ...
                // OPTIMISTIC UPDATE
                updateCache('leads', (leads) =>
                    leads.map(l => l.id === selectedLead.id ? updatedLead : l)
                );
                ...
                // OPTIMISTIC UPDATE
                updateCache('leads', (leads) => [...leads, newLead]);
                ...
                if (token && window.api?.createLead) {
                    try {
                        ...
                        if (savedLead && savedLead.id) {
                            updateCache('leads', (leads) =>
                                leads.map(l => l.id === Date.now().toString() ? savedLead : l)
                            );
                        }
                    } catch (apiError) {
                        console.error('⚠️ Background sync failed:', apiError);
    ...
    const handleDeleteClient = async (clientId) => {
        try {
            // OPTIMISTIC UPDATE
            updateCache('clients', (clients) => clients.filter(c => c.id !== clientId));
    ...
    const handleDeleteLead = async (leadId) => {
        try {
            // OPTIMISTIC UPDATE
            updateCache('leads', (leads) => leads.filter(l => l.id !== leadId));
    ...
    const convertLeadToClient = (lead) => {
        const newClient = {
            id: Math.max(0, ...clients.map(c => parseInt(c.id) || 0)) + 1,
            name: lead.name,
        ...
        // Optimistic updates
        updateCache('clients', (clients) => [...clients, newClient]);
        updateCache('leads', (leads) => leads.filter(l => l.id !== lead.id));
