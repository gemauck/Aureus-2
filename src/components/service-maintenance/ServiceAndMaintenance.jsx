...
function JobCardSafetyCultureThumbnailService({
  ...
  return (
    <div className={`overflow-hidden rounded-xl border ${border}`}>
      <div className={`truncate px-2 py-1 text-[11px] ${fnColor}`} title={filename}>
        {filename}
      </div>
...
const ServiceAndMaintenance = () => {
    ...
    if (Array.isArray(photos)) {
      photos.forEach((p, idx) => {
        if (
          ...
          fromPhotos.push({ url: p.url.trim(), idx });
        }
      });
    }
  ...
  const handleDownloadJobCardPdf = useCallback(async () => {
    ...
    const formatPdfDate = (value) => {
      const dt = parseJobCardDateValue(value);
      if (!dt) return '—';
                    ...
                    {signatureAttachmentItems.length > 1 ? (
                      <div>
                        <div className={`text-[11px] font-semibold uppercase mb-2 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                          Additional signature copies
                        </div>
                        <div className="flex flex-wrap gap-3">
                          {signatureAttachmentItems.slice(1).map((s) => (
                            <figure
                              key={`sig-${s.idx}`}
