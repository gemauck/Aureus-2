/**
 * Browser-friendly script to add stock items to inventory
 * Run this in the browser console while logged into the ERP system
 * 
 * Usage: Copy and paste this entire script into the browser console
 */

(async function addStockItemsToInventory() {
  // Stock items data
  const stockItems = [
  ...
  if (!token) {
    ...
    alert('Please log in to the ERP system first, then run this script again.');
    return;
  }
  ...
  for (let i = 0; i < stockItems.length; i++) {
    ...
    const inventoryData = {
      ...
      reorderPoint: Math.max(1, Math.floor(item.quantity * 0.2)),
      reorderQty: Math.max(10, Math.floor(item.quantity * 0.3)),
      location: '',
      supplier: '',
    ...
    try {
      ...
      const sku = result.data?.item?.sku || 'N/A';
      console.log(`✅ [${i + 1}/${stockItems.length}] Added: ${item.partNumber} (SKU: ${sku})`);
      successCount++;
  ...
  alert(`✅ Added ${successCount} items successfully!\n❌ ${errorCount} items failed.\n\nCheck console for details.`);
})();
