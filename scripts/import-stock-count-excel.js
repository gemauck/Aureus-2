 ...
 *
 * Usage: node scripts/import-stock-count-excel.js "/path/to/file.xlsx" [--dry-run] [--only-location=PMB] [--needs-catalog-review]
...
// Strips leading currency markers (e.g. R6.077, ZAR12.5)
...
/** Pietermaritzburg / PMB office — same heuristics as scripts/compare-inventory-to-xlsx-pmb.mjs */
...
/**
 * When --only-location is set: prefer existing DB location (code match, then PMB/Pietermaritzburg heuristics), else create.
 */
async function resolveForcedStockLocation(prisma, onlyLocation) {
  const token = String(onlyLocation || '').trim()
  ...
  if (upper === 'PMB' || token.toUpperCase().includes('PIETER')) {
  ...
  return ensureLocation(prisma, token)
}
...
async function main() {
  ...
  const locationMap = {}
  if (onlyLocation) {
    const forced = await resolveForcedStockLocation(prisma, onlyLocation)
    if (!forced) {
      console.error('Could not resolve --only-location=', onlyLocation)
      await prisma.$disconnect()
      process.exit(1)
    }
    locationMap[normalizeCode(onlyLocation)] = forced
    locationMap[String(onlyLocation).trim()] = forced
    locationMap[''] = forced
    console.log('   Forced location:', forced.code, '—', forced.name)
  } else {
    const locationsToEnsure = [...locationCodes]
