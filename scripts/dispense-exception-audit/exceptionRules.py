# Recompute dispense exception flags from transaction rows (InsightWare rules).
"""Exception rule engine for dispense exception audit."""
...
# Canonical reason fragments (workbook text)
...
REASON_ODO_JUMP = 'Odo difference > 50 hrs, Consecutive dispenses within 60 minutes, Actual time elapsed between dispenses < Odo time elapsed'
REASON_ODO_JUMP_120 = 'Odo difference > 50 hrs, Consecutive dispenses within 120 minutes, Actual time elapsed between dispenses < Odo time elapsed'
...
class RuleFlags:
    ...
    def reason_120_parts(self) -> list[str]:
        ...
        if self.odo_jump_50:
            return [REASON_ODO_JUMP.replace('60 minutes', '120 minutes')]
        if self.over_tank_detail:
            parts.append(self.over_tank_detail.replace('60 minutes', '120 minutes'))
        return parts
...
def compute_flags_for_asset(asset_rows: list[dict[str, Any]]) -> list[ComputedTransaction]:
    ...
    rows = sorted(
        asset_rows,
        key=lambda r: (r.get('date_time') or datetime.min, str(r.get('transaction_id') or '')),
    )
    ...
    for i, row in enumerate(rows):
        ...
        minutes_since_prev = _minutes_between(prev.get('date_time') if prev else None, dt)
        minutes_to_next = _minutes_between(dt, next_row.get('date_time') if next_row else None)
        ...
        # Same-timestamp zero-usage split: flag the odo<=0 row when paired at identical time.
        if (
            next_row
            and minutes_to_next == 0
            and flags.odo_non_positive
            and (next_row.get('total_usage_num') or 0) > 0
        ):
            flags.consec_60 = True
            flags.consec_120 = True
        ...
        # fill outside hour: gap (60,120] from previous OR >60 from batch start within consec chain
        if prev and minutes_since_prev is not None:
            if 60 < minutes_since_prev <= 120:
                flags.fill_outside_hour = True
            elif flags.consec_60 and minutes_since_batch is not None and minutes_since_batch > 60:
                flags.fill_outside_hour = True

        # odo jump >50 with wall-clock < meter elapsed
        if (
            prev
            and flags.consec_60
            and usage is not None
            and usage > 50
            and minutes_since_prev is not None
            and minutes_since_prev < usage * 60
        ):
            flags.odo_jump_50 = True

        # cumulative litres > tank in consecutive batch
        ...
        results.append(
            ComputedTransaction(
                ...
                cumulative_litres_in_batch=(
                    sum(float(r.get('litres') or 0) for r in rows[batch_start_idx : i + 1])
                    if batch_start_idx is not None
                    else None
                ),
            )
        )

        if not flags.consec_60:
            batch_start_idx = i
            batch_start_time = dt
...
def compute_all(transactions: list[dict[str, Any]]) -> dict[str, ComputedTransaction]:
    ...
    for asset_rows in by_asset.values():
        for item in compute_flags_for_asset(asset_rows):
            computed[item.transaction_id] = item
    return computed
...
def compare_reason(reported: str | None, expected: str | None) -> dict[str, Any]:
    reported_parts = normalize_reason_text(reported)
    expected_parts = normalize_reason_text(expected)
    missing = reported_parts - expected_parts
    extra = expected_parts - reported_parts
    return {
        'match': not missing and not extra,
        'missing_from_recompute': sorted(missing),
        'extra_in_recompute': sorted(extra),
        'reported': reported,
        'expected': expected,
    }


def match_rate(transactions: list[dict[str, Any]], computed: dict[str, ComputedTransaction], column: str) -> dict[str, Any]:
    total_flagged = 0
    matched = 0
    ...
    for row in transactions:
        ...
        if not reported:
            continue
        total_flagged += 1
        comp = computed.get(str(row.get('transaction_id')))
        if not comp:
            mismatches.append({'transaction_id': row.get('transaction_id'), 'issue': 'not_computed'})
            continue
        ...
        if cmp['match']:
            ...
            mismatches.append({
                ...
                'reported': reported,
                'expected': expected,
                'missing': cmp['missing_from_recompute'],
                'extra': cmp['extra_in_recompute'],
            })

    pct = (matched / total_flagged * 100) if total_flagged else 100.0
    return {
