...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      // blob: for object-URL previews (e.g. QR/camera flows); data: already allowed
...
// GET review-cell-activity: never 500/502, always 200 (empty timeline or data)
app.get('/api/projects/:id/review-cell-activity', async (req, res) => {
  const emptyPayload = JSON.stringify({ data: { timeline: [] } })
  let timeoutId = null
  const sendEmpty = () => {
    try {
  ...
  try {
    const handlerPath = path.join(apiDir, 'projects', '[id]', 'review-cell-activity.js')
    const handler = await loadHandler(handlerPath)
    const result = handler(req, res)
    if (result != null && typeof result.then === 'function') {
      await result.catch(() => {
...
app.all('/api/public/web-error-report', async (req, res, next) => {
  try {
    const handler = await loadHandler(path.join(apiDir, 'public', 'web-error-report.js'))
    if (!handler) return res.status(404).json({ error: 'API endpoint not found' })
    return handler(req, res)
  } catch (e) {
    console.error('❌ Public web-error-report API error:', e)
    return next(e)
  }
})
...
// Public job card update by id (PATCH) — register before /api/public/jobcards
app.all('/api/public/jobcards/:id', async (req, res, next) => {
  try {
    const handler = await loadHandler(path.join(apiDir, 'public', 'jobcards-by-id.js'))
    if (!handler) return res.status(404).json({ error: 'API endpoint not found' })
    return handler(req, res)
  } catch (e) {
    console.error('❌ Public jobcards/:id API error:', e)
    return next(e)
  }
})

app.all('/api/public/jobcards-calendar.ics', async (req, res, next) => {
  try {
    const handler = await loadHandler(path.join(apiDir, 'public', 'jobcards-calendar.js'))
    if (!handler) return res.status(404).json({ error: 'API endpoint not found' })
    return handler(req, res)
  } catch (e) {
    console.error('❌ Public jobcards calendar API error:', e)
    return next(e)
  }
})
...
app.post('/api/tools/sparrow-to-gilbarco', (req, res) =>
  runLoadedHandler(req, res, path.join(apiDir, 'tools', 'sparrow-to-gilbarco.js'), {
    timeoutMs: SPARROW_TO_GILBARCO_TIMEOUT_MS,
    routeLabel: 'Sparrow to Gilbarco',
  })
)
...
app.use('/api', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!res.headersSent && !res.writableEnded) {
        ...
        const isGetCellActivity = req.method === 'GET' && (
          (req.url && (req.url.includes('document-collection-cell-activity') || req.url.includes('review-cell-activity'))) ||
          (req.originalUrl && (req.originalUrl.includes('document-collection-cell-activity') || req.originalUrl.includes('review-cell-activity'))) ||
          (handlerPath && (handlerPath.includes('document-collection-cell-activity') || handlerPath.includes('review-cell-activity')))
        )
        if (isGetActivity) {
    ...
    const isGetCellActivity = req.method === 'GET' && (
      (req.url && (req.url.includes('document-collection-cell-activity') || req.url.includes('review-cell-activity'))) ||
      (req.originalUrl && (req.originalUrl.includes('document-collection-cell-activity') || req.originalUrl.includes('review-cell-activity'))) ||
      (typeof handlerPath === 'string' && (handlerPath.includes('document-collection-cell-activity') || handlerPath.includes('review-cell-activity')))
    )
