...
async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST' && isListOrCreate) {
      ...
      const parsedProb = Number.parseInt(String(body.probability), 10)
      const safeProbability = Number.isFinite(parsedProb) ? parsedProb : 0
      const lastContactRaw = body.lastContact ? new Date(body.lastContact) : new Date()
      const lastContact = Number.isNaN(lastContactRaw.getTime()) ? new Date() : lastContactRaw
      const clientData = {
        name: body.name,
    ...
    if (pathSegments.length === 2 && pathSegments[0] === 'clients' && id) {
      ...
      if (req.method === 'PATCH') {
        ...
        if (body.contacts !== undefined) {
          ...
          // Keep JSON fields in sync so fallback reads don't bring back deleted contacts.
          jsonFieldsToUpdate.contacts = JSON.stringify(contactsArray)
          jsonFieldsToUpdate.contactsJsonb = contactsArray

          // Normalized ClientContact table is the source of truth.
