...
async function handler(req, res) {
  try {
    ...
    if (!isReceiptCaptureAdmin(user)) {
      return forbidden(res, 'Only administrators can modify cost centres.')
    }

    if (req.method === 'GET') {
      const row = await prisma.receiptCostCenter.findUnique({ where: { id } })
      if (!row) return notFound(res)
      return ok(res, { costCenter: serialize(row) })
    }
    ...
    if (req.method === 'DELETE') {
      try {
        await prisma.receiptCostCenter.delete({ where: { id } })
    ...
    console.error('receipt-cost-centers/[id]:', e)
